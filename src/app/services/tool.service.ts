...
export class ToolService {
  ...
  team1 = new Team([], [], [], 0, 0, 0, 0, 0, 0, 0, 0);
  team2 = new Team([], [], [], 0, 0, 0, 0, 0, 0, 0, 0);
  ...
  computeFinalValues() {
    ...
    if (this.team1.value >= this.team2.value) {
      ...
      let finalSecondTeam = JSON.parse(JSON.stringify(this.getSecondTeam(this.team1.value, this.team2.value, this.team1.bonus, this.team2.bonus, this.team1.extra, this.team2.extra, this.team2.finalPlayers, this.team2.bonusList, this.team1.quot, this.team2.quot)))
      this.team2.finalPlayers = []
      this.team1.finalPlayers = []
      ...
      let finalFirstTeam = JSON.parse(JSON.stringify(this.getSecondTeam(this.team2.value, this.team1.value, this.team2.bonus, this.team1.bonus, this.team2.extra, this.team1.extra, this.team1.finalPlayers, this.team1.bonusList, this.team2.quot, this.team1.quot)))
    ...
    if (this.team1.repaid != 0 || this.team2.repaid != 0) {
          ...
          this.team1.finalPlayers.forEach(
            player => {
              player.finalValue = this.getPlayerMaxValue(player) / player.quot * player.finalQuot +
                ...
                * player.finalQuot / this.team1.finalQuot
            }
          )
        }
      } else { //2.3.1.2
        if (this.team1.finalValue + this.team2.bonus + this.team2.repaid >
          this.team2.finalValue + this.team1.bonus + this.team1.repaid) {
          if (this.team1.value + this.team2.bonus + this.team2.repaid >
            this.team2.value + this.team1.bonus + this.team1.repaid) {
            this.team1.finalPlayers.forEach(
              player => {
            ...
            this.team1.finalPlayers.forEach(
              player => {
                if ((-this.team2.finalValue - this.team1.bonus - this.team1.repaid
      ...
      if (this.team2.value == this.team2.finalValue) {
          ...
          this.team2.finalPlayers.forEach(
            player => {
              player.finalValue = this.getPlayerMaxValue(player) / player.quot * player.finalQuot +
                (this.team1.value + this.team2.bonus + this.team2.repaid
                  - this.team2.value - this.team1.bonus - this.team1.repaid)
                * player.finalQuot / this.team2.finalQuot
            }
          )
        }
      } else { //2.3.1.2
        if (this.team2.finalValue + this.team1.bonus + this.team1.repaid >
          this.team1.finalValue + this.team2.bonus + this.team2.repaid) {
          if (this.team2.value + this.team1.bonus + this.team1.repaid >
            this.team1.value + this.team2.bonus + this.team2.repaid) {
            this.team2.finalPlayers.forEach(
              player => {
                if (this.getPlayerMaxValue(player) / player.quot * player.finalQuot -
  ...
  getSecondTeam(teamBiggerValue: number, teamSmallerValue: number, teamBiggerBonus: number, teamSmallerBonus: number, teamBiggerExtra: number, teamSmallerExtra: number,
    players: Player[], bonusSequence: number[], teamBiggerQuot: number, teamSmallerQuot: number) {
    //SECOND BIG TEAM
    if (teamSmallerBonus > teamBiggerBonus) {
      players.forEach(
        (player, index) => {
          if (player.contractType == "lend" || (player.contractType === 'lendR' && player.repaid === "no")) {
            player.finalValue = player.value + ((12 / +player.contractLength) / 0.8) * (
              (teamBiggerValue - teamSmallerValue) * player.quot / teamSmallerQuot +
              (teamSmallerBonus - teamBiggerBonus) * bonusSequence[index] / teamSmallerBonus)
          } else if (player.contractType === 'lendR' || player.contractType === 'lendO') {
            player.finalValue = player.value + (12 / +player.contractLength) * (
              (teamBiggerValue - teamSmallerValue) * player.quot / teamSmallerQuot +
              (teamSmallerBonus - teamBiggerBonus) * bonusSequence[index] / teamSmallerBonus)
          }
          else {
      ...
      players.forEach(
        (player, index) => {
          if (player.contractType == "lend" || (player.contractType === 'lendR' && player.repaid === "no")) {
            player.finalValue = player.value + ((12 / +player.contractLength) / 0.8) * (
              (teamBiggerValue - teamSmallerValue) * player.quot / teamSmallerQuot +
              (teamSmallerBonus - teamBiggerBonus) * player.quot / teamSmallerQuot)
          } else if (player.contractType === 'lendR' || player.contractType === 'lendO') {
            player.finalValue = player.value + (12 / +player.contractLength) * (
              (teamBiggerValue - teamSmallerValue) * player.quot / teamSmallerQuot +
              (teamSmallerBonus - teamBiggerBonus) * player.quot / teamSmallerQuot)
